...
export const Dashboard: React.FC<DashboardProps> = ({ onRunAnalysis, initialPreset, onSaveToVault }) => {
  ...
  const getVerdictColors = (score: number) => {
    if (score >= 80) return { bg: 'rgba(0,255,122,0.07)', border: 'rgba(0,255,122,0.2)', accent: '#00FF7A', glow: 'rgba(0,255,122,0.3)', arc: '#00D668', label: 'VERIFIED AUTHENTIC' };
    if (score >= 50) return { bg: 'rgba(252,211,77,0.07)', border: 'rgba(252,211,77,0.2)', accent: '#FCD34D', glow: 'rgba(252,211,77,0.3)', arc: '#D97706', label: 'SUSPICIOUS' };
    return { bg: 'rgba(251,113,133,0.07)', border: 'rgba(251,113,133,0.2)', accent: '#FB7185', glow: 'rgba(251,113,133,0.3)', arc: '#E11D48', label: 'LIKELY COUNTERFEIT' };
  };
      ...
      <div className="w-full mx-auto px-4 sm:px-6 lg:px-10 xl:px-14 space-y-8">
              ...
              <button
                ...
                style={
                  itemName === preset.title
                    ? { background: 'linear-gradient(135deg, #00FF7A, #00D668)', color: '#040907', border: 'none', boxShadow: '0 0 16px rgba(0,255,122,0.3)' }
                    : { background: 'rgba(0,255,122,0.05)', border: '1px solid rgba(0,255,122,0.1)', color: 'var(--text-muted)' }
                }
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-7">
          ...
          <div className="lg:col-span-4 space-y-5">
            ...
            {/* Image Upload */}
            <div className="p-6 rounded-2xl space-y-4" style={{ background: 'var(--bg-card)', border: '1px solid rgba(0,255,122,0.08)' }}>
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  style={{ background: 'rgba(0,255,122,0.08)', border: '1px solid rgba(0,255,122,0.15)', color: 'var(--green-accent-from)' }}
                >
              ...
              <div
                ...
                style={{ borderColor: 'rgba(0,255,122,0.15)', background: 'rgba(0,255,122,0.02)' }}
                onMouseEnter={e => (e.currentTarget.style.borderColor = 'rgba(0,255,122,0.35)')}
                onMouseLeave={e => (e.currentTarget.style.borderColor = 'rgba(0,255,122,0.15)')}
              >
                {imageUrl ? (
                  <div className="relative w-full h-full min-h-[200px] rounded-lg overflow-hidden flex items-center justify-center">
                    ...
                    <div
                      ...
                      style={{ background: 'rgba(4,9,7,0.7)' }}
                    >
                      <button
                        type="button"
          ...
          <div className="lg:col-span-8 space-y-5">
            {isLoading ? (
              ...
              <div
                ...
                style={{ background: 'var(--bg-card)', border: '1px solid rgba(0,255,122,0.1)' }}
              >
                <div className="absolute inset-0 hex-grid-bg opacity-50 pointer-events-none" />
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] h-[300px] rounded-full blur-3xl pointer-events-none animate-breathe" style={{ background: 'rgba(0,255,122,0.05)' }} />

                <div className="relative w-28 h-28">
                  <div className="absolute inset-0 rounded-full border-2 border-t-[var(--green-accent-from)] animate-spin" style={{ borderColor: 'rgba(0,255,122,0.15)', borderTopColor: 'var(--green-accent-from)' }} />
                  ...
                  return (
                    <>
                          ...
                          <div className="flex items-center gap-5 shrink-0">
                            <div className="relative w-28 h-28 sm:w-32 sm:h-32 animate-halo-pulse">
                              <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
                                <circle cx="50" cy="50" r="40" fill="none" strokeWidth="8" stroke="rgba(255,255,255,0.06)" />
                                <circle cx="50" cy="50" r="44" fill="none" strokeWidth="1.5" stroke="rgba(255,255,255,0.04)" />
                                <circle
                                  cx="50" cy="50" r="40"
                                  fill="none"
                                  strokeWidth="8"
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-12 gap-5">
                        {/* Confidence */}
                        <div className="sm:col-span-5 p-6 rounded-2xl space-y-4" style={{ background: 'var(--bg-card)', border: '1px solid rgba(0,255,122,0.08)' }}>
                          <div className="flex items-center justify-between">
                        ...
                        {/* Detailed Metrics */}
                        <div className="sm:col-span-7 p-6 rounded-2xl space-y-4" style={{ background: 'var(--bg-card)', border: '1px solid rgba(0,255,122,0.08)' }}>
                          ...
                          {analysisResult.imageUrl ? (
                            <div className="space-y-3.5">
                              {[
                                { label: 'Stitching Pitch', value: analysisResult.detailedScores.stitchingQuality, color: 'var(--green-accent-from)', glow: 'rgba(0,255,122,0.4)' },
                                { label: 'Hardware Engraving', value: analysisResult.detailedScores.hardwareAuthenticity, color: '#818CF8', glow: 'rgba(129,140,248,0.4)' },
                            ...
                            <div className="grid grid-cols-2 gap-4 pt-1">
                              {[
                                { label: 'Linguistic Perplexity', value: analysisResult.detailedScores.reviewPerplexity, sub: 'Organic syntactic entropy', color: '#818CF8', bg: 'rgba(129,140,248,0.06)', border: 'rgba(129,140,248,0.12)' },
                                { label: 'Sentiment Coherence', value: analysisResult.detailedScores.reviewSentimentAlignment, sub: 'Authentic buyer tone', color: 'var(--green-accent-from)', bg: 'rgba(0,255,122,0.06)', border: 'rgba(0,255,122,0.12)' },
                      ...
                      {analysisResult.imageUrl && (
                        <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--bg-card)', border: '1px solid rgba(0,255,122,0.08)' }}>
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-6 sm:px-8 py-5" style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                            <div className="flex items-center gap-3">
                              <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: 'rgba(0,255,122,0.08)', border: '1px solid rgba(0,255,122,0.15)' }}>
                            ...
                            {/* Mode toggles */}
                            <div className="flex items-center p-1 rounded-xl gap-0.5 text-[11px] font-black" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.06)' }}>
                              {(['heatmap', 'bounding_boxes', 'raw'] as const).map((mode) => (
                                <button
                                  ...
                                  style={viewMode === mode
                                    ? { background: 'rgba(0,255,122,0.12)', color: 'var(--green-accent-from)', border: '1px solid rgba(0,255,122,0.2)' }
                                    : { color: 'var(--text-dim)' }
                                  }
                                >
                          ...
                          {/* Canvas */}
                          <div className="relative w-full min-h-[360px] flex items-center justify-center p-6" style={{ background: 'rgba(0,0,0,0.3)', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                            <img
                            ...
                            />
                            {viewMode === 'heatmap' && (
                              <div className="absolute inset-0 rounded-b-none pointer-events-none" style={{ background: 'linear-gradient(135deg, rgba(0,255,122,0.08), rgba(129,140,248,0.06), rgba(252,211,77,0.04))', mixBlendMode: 'screen' }} />
                            )}
                            {viewMode !== 'raw' && analysisResult.heatmapPoints.map(point => {
                              ...
                              const col = isCritical ? '#FB7185' : '#00FF7A';
                              return (
                                <div
                                  key={point.id}
                                ...
                                >
                                  <span className="text-[8px] font-black px-1.5 py-0.5 rounded whitespace-nowrap" style={{ background: col, color: '#040907', marginTop: '-14px', marginLeft: '-4px' }}>
                                    {point.label}
                                  </span>
                                </div>
                              );
                        ...
                        <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--bg-card)', border: '1px solid rgba(129,140,248,0.12)' }}>
                            ...
                            <span
                              ...
                              style={
                                analysisResult.fakeReviewProbability > 50
                                  ...
                                  : { background: 'rgba(0,255,122,0.1)', border: '1px solid rgba(0,255,122,0.25)', color: 'var(--green-accent-from)' }
                              }
                            >
                          ...
                          <div className="p-6 sm:p-8 space-y-6">
                            ...
                            <div className="relative rounded-xl p-5" style={{ background: 'rgba(129,140,248,0.05)', border: '1px solid rgba(129,140,248,0.1)' }}>
                              ...
                              <div className="text-[10px] font-black uppercase tracking-widest mb-2" style={{ color: '#818CF8' }}>Analyzed Review</div>
                            ...
                            <div className="p-5 rounded-xl space-y-4" style={{ background: 'rgba(129,140,248,0.05)', border: '1px solid rgba(129,140,248,0.1)' }}>
                              <div className="flex items-center justify-between">
                                ...
                                <span className="text-3xl font-black font-mono" style={{ color: analysisResult.fakeReviewProbability > 50 ? '#FB7185' : '#818CF8' }}>
                                ...
                                <div className="pt-3 space-y-2" style={{ borderTop: '1px solid rgba(129,140,248,0.1)' }}>
                                  ...
                                  {analysisResult.reviewFlags.map((flag, idx) => (
                                    <div key={idx} className="flex items-start gap-2.5 p-3 rounded-xl text-[12px]" style={
                                      flag.severity === 'high'
                                        ? { background: 'rgba(251,113,133,0.07)', border: '1px solid rgba(251,113,133,0.15)' }
                                        : { background: 'rgba(252,211,77,0.07)', border: '1px solid rgba(252,211,77,0.15)' }
                                    }>
                                      <AlertTriangle className="w-3.5 h-3.5 shrink-0 mt-0.5" style={{ color: flag.severity === 'high' ? '#FB7185' : '#FCD34D' }} />
                                      <div>
                              ...
                              {(!analysisResult.reviewFlags || analysisResult.reviewFlags.length === 0) && (
                                <div className="pt-3" style={{ borderTop: '1px solid rgba(0,255,122,0.08)' }}>
                                  <div className="flex items-center gap-2 text-[12px] font-bold p-3 rounded-xl" style={{ background: 'rgba(0,255,122,0.07)', border: '1px solid rgba(0,255,122,0.15)', color: 'var(--green-accent-from)' }}>
                                ...
                                <ul className="p-5 space-y-3">
                                  {analysisResult.xaiReasoning.map((reason, idx) => (
                                    <li key={idx} className="flex items-start gap-3">
                                      <span className="w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-black shrink-0 mt-0.5" style={{ background: 'rgba(255,255,255,0.06)', color: 'var(--text-dim)' }}>{idx + 1}</span>
                                      <span className="text-[13px] leading-relaxed" style={{ color: 'var(--text-muted)' }}>{reason}</span>
                                    </li>
                                  ))}
                                </ul>
                              </div>
                              <div className="rounded-xl overflow-hidden" style={{ border: '1px solid rgba(0,255,122,0.1)' }}>
                                <div className="px-5 py-3.5 flex items-center gap-2" style={{ borderBottom: '1px solid rgba(0,255,122,0.08)', background: 'rgba(0,255,122,0.04)' }}>
              ...
              <div
                ...
                style={{ background: 'var(--bg-card)', border: '1px solid rgba(0,255,122,0.07)' }}
              >
                <div className="absolute inset-0 hex-grid-bg opacity-40 pointer-events-none" />
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] h-[300px] rounded-full blur-3xl pointer-events-none animate-breathe" style={{ background: 'rgba(0,255,122,0.04)' }} />
                <div
                ...
                <div className="flex flex-wrap justify-center gap-3 relative z-10 mt-2">
                  {[
                    ...
                    <div key={label} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold" style={{ background: 'rgba(0,255,122,0.06)', border: '1px solid rgba(0,255,122,0.1)', color: 'var(--text-muted)' }}>
      ...
      {showCertificateModal && analysisResult && (
        ...
        >
          <div
            ...
            style={{ background: 'var(--bg-surface-2)', border: '1px solid rgba(0,255,122,0.15)', boxShadow: '0 40px 100px rgba(0,0,0,0.8), 0 0 60px rgba(0,255,122,0.08)' }}
          >
            <button
            ...
            <div className="text-center space-y-3 pb-6" style={{ borderBottom: '1px solid rgba(0,255,122,0.08)' }}>
              <div className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto animate-glow-pulse" style={{ background: 'rgba(0,255,122,0.1)', border: '1px solid rgba(0,255,122,0.25)' }}>
            ...
            <div className="space-y-3 text-[13px]">
              {[
                ...
                <div key={label} className="flex justify-between items-center p-4 rounded-xl" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
                  <span style={{ color: 'var(--text-muted)' }}>{label}:</span>
                  <span
                    className={mono ? 'font-mono' : 'font-bold'}
                    style={{
                      color: highlight
                        ? (score && score >= 80 ? 'var(--green-accent-from)' : '#FB7185')
                        : mono ? '#818CF8' : 'var(--text-primary)',
            ...
            <div className="p-5 rounded-xl flex items-center justify-between" style={{ background: 'rgba(0,255,122,0.06)', border: '1px solid rgba(0,255,122,0.15)' }}>
              <div>
