...
// Bulletproof multi-engine live scraping + Gemini 2.5/3.5 Grounding & Forensics
...
function extractBestPrice(text: string): string {
  ...
  // 2. Generic price matches, filtering out trivial numbers (like ₹40 delivery or ₹0)
  const allMatches = [
...
async function scrapeUniversalProduct(cleanUrl: string, asin?: string) {
  ...
  // Engine 2: OpenGraph Metadata via Microlink
...
export async function runUniversalGeminiForensics(rawUrl: string): Promise<any> {
  ...
  // Single-pass deep forensic analysis with Gemini
...
Return ONLY valid JSON matching this schema:
{
  ...
  // Final Image Resolution — Ensure we have a high-res valid image
  ...
  // Normalize all xaiReasoning strings to use the exact resolvedPrice
  ...
  fixedXaiReasoning = fixedXaiReasoning.map((reason: string) => {
    // Replace any conflicting currency values with resolvedPrice
